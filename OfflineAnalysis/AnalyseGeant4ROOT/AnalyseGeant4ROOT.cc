...
template<typename T> void WriteVectorChannelsToFile( TTree *t1, vector<int> *ActivePMT, vector<T> *v, string name){
	...
	if(DEBUG && (name == "PmtMedianHitTimes"))
		cout<<" Number of entries in t1 "<< n << endl;;
...
void FindPeaks(vector<float> &Waveform, vector<float> &Peaks){
	...
	if(ChannelSize > 0){
		...
		for (int p=0;p<nfound;p++) {
			...
			Peaks.push_back( xp );   
			if(DEBUG)
	  			cout<< "Bin location:"<<xp << " Bin value:" << yp << endl;	
		}
...
// This function suggests a set of candidate channels in each of the stacks/layers, depending on which OfflineTriggerStrategy you want to perform
// Those candidate channels are then tested outside of this function to see if they have activity that lies within the coincidence threshold
...
void GetNextCandidate(bool &TestNextCandidate, vector<int> &CandidateChannels, int ActiveChannel, int &CandidateCounter){
	...
	for(std::vector<int>::iterator y1 = NN.begin() ; y1 != NN.end(); ++y1){
		...
		for(std::vector<int>::iterator y2 = NN2y.begin() ; y2 != NN2y.end(); ++y2){	
			...
			if( ChannelsToCoordinates( ActiveChannel )[2] == 0 && *z1==-1)
				continue;
			if( ChannelsToCoordinates( ActiveChannel )[2] == NBlocks[2]-1 && *z1==1)
				continue;
			if( ChannelsToCoordinates( ActiveChannel )[1] == 0 && *y1==-1)
				continue;
			if( ChannelsToCoordinates( ActiveChannel )[1] == NBlocks[1]-1 && *y1==1)
				continue;


			if( ChannelsToCoordinates( layer1 )[2] == 0 && *z2==-1)
				continue;
			if( ChannelsToCoordinates( layer1 )[2] == NBlocks[2]-1 && *z2==1)
				continue;
			if( ChannelsToCoordinates( layer1 )[1] == 0 && *y2==-1)
				continue;
			if( ChannelsToCoordinates( layer1 )[1] == NBlocks[1]-1 && *y2==1)
				continue;
	...
	for(unsigned int i=0; i < ActiveChannels.size(); i++) {
		...
		if(OfflineTriggerStrategy == "ThreeFoldAnywhere"){
			...
			if( (ActiveChannels[i] >= NBlocksPerStack) && (ActiveChannels[i] < 2* NBlocksPerStack))
				GoodEvent1 = true;
			if(ActiveChannels[i] >= 2*NBlocksPerStack)
				GoodEvent2 = true;
		...
		while( TestNextCandidate == true ){
			...
			if(recordEvent == true){
				...
				if(DEBUG)
					cout << " This is ATWD offset between concecutive layers " << ATWDOffset << endl;
...
int CoincidencesManager( TTree *t1, vector<int> *Translation, vector<float> *v){ 
	...
	for(int i = EventRangeBegin; i < n; i++){
		...
		if (  IsCoincidence( PeakLocations, ActiveChannels, MaxSamples ) )
			SignalCounts++;
...
void GeantSensitivities(string PathName, vector<string> Particle, string ConfigType, string ConfigDetails, bool OutputWaveformsToFile ){
	...
	for(unsigned long i = 0; i < Particle.size(); i++ ){
		...
		remove( EfficiencyOutputName.c_str() );
			...
			if(OutputWaveformsToFile)
				WriteVectorToFile(t1, WaveformLengthPerChannel, WaveformVoltage, "WaveformVoltage."+fileName);
...
int main(int argc, char* argv[]) {
	...
	if (DoSensitivities)
		GeantSensitivities(PathName, Particle, ConfigType, ConfigDetails,OutputWaveformsToFile  );


	// If we are interested in plotting and studying the truth G4 information that is precalculated in the simulation
	if (DoHistograms)
		KinematicPlots(PathName, PlotParticle, ConfigType, PlotCharge, PlotMass);
