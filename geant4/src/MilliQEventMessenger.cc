...
// ********************************************************************
//
// $Id: MilliQEventMessenger.cc 70256 2013-05-28 07:29:30Z gcosmo $
//
/// \file optical/MilliQ/src/MilliQEventMessenger.cc
/// \brief Implementation of the MilliQEventMessenger class
//
//
  ...
  : fMilliQEvent(event)
{
  ...
  fVerboseCmd = new G4UIcmdWithAnInteger("/MilliQ/eventVerbose",this);
  fVerboseCmd->SetGuidance("Set the verbosity of event data.");
  fVerboseCmd->SetParameterName("verbose",true);
  fVerboseCmd->SetDefaultValue(1);

  fPmtThresholdCmd = new G4UIcmdWithAnInteger("/MilliQ/pmtThreshold",this);
  fPmtThresholdCmd->SetGuidance("Set the pmtThreshold (in # of photons)");

  fForceDrawPhotonsCmd=new G4UIcmdWithABool("/MilliQ/forceDrawPhotons",this);
  fForceDrawPhotonsCmd->SetGuidance("Force drawing of photons.");
  fForceDrawPhotonsCmd
    ->SetGuidance("(Higher priority than /MilliQ/forceDrawNoPhotons)");

  fForceDrawNoPhotonsCmd=new G4UIcmdWithABool("/MilliQ/forceDrawNoPhotons",this);
  fForceDrawNoPhotonsCmd->SetGuidance("Force no drawing of photons.");
  fForceDrawNoPhotonsCmd
    ->SetGuidance("(Lower priority than /MilliQ/forceDrawPhotons)");
}
